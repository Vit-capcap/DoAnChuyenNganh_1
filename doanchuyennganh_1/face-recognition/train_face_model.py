# """
# Train SVM model nhận diện khuôn mặt.
...
#         print(f"Không tìm thấy embeddings file: {EMBEDDINGS_FILE}")
#         print("Hãy chạy trước: python extract_embeddings.py")
#         return
...
#     if len(X) == 0:
#         print("Không có dữ liệu embedding để train.")
#         return
...
#     class_counts = {label: int(np.sum(y == label)) for label in encoder.classes_}
#     min_count = min(class_counts.values())

#     # Nếu mỗi class đủ mẫu thì tách train/test, không thì train toàn bộ.
#     if len(encoder.classes_) >= 2 and min_count >= 2 and len(X) >= 10:
...
#         ))
#     else:
#         print("\n[WARN] Dữ liệu ít, train toàn bộ dataset, không tách test.")
