...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    // Set tạm từ localStorage trước (ưu tiên hiển thị nhanh)
    setUserName(user.full_name || user.username || "Bạn");
    if (user.avatar) setAvatarSrc(resolveImageUrl(user.avatar));

    // Gọi API profile để lấy full_name và avatar thật
    if (user.student_id) {
      getMyStudentProfile(user.student_id)
        .then((res) => {
          if (res && res.success && res.data) {
            ...
            setAvatarSrc(resolveImageUrl(p.avatar));
        ...
        .catch((err) => {
          // Lỗi API profile không crash dashboard, giữ giá trị tạm
    ...
    <div className="bg-slate-50 min-h-screen font-[Inter] text-gray-800">
      ...
      <aside className="fixed left-0 top-0 w-[280px] h-screen bg-[#111827] text-white hidden md:flex flex-col z-50">
        ...
        <div className="flex-1 p-4 space-y-2">
          ...
          <div onClick={() => handleNotUnimplemented("Face Registration")} className="cursor-pointer">
        ...
        <div className="p-4 border-t border-white/10">
          <button
            type="button"
            onClick={() => handleNotUnimplemented("Help Support")}
            className="w-full bg-white/10 hover:bg-white/20 transition rounded-xl py-3 mb-3"
          >
            Help Support
          </button>
      ...
      <header className="fixed top-0 right-0 md:left-[280px] h-[80px] bg-white/80 backdrop-blur-lg border-b border-gray-200 flex items-center justify-between px-6 z-40">
        ...
        <div className="flex items-center gap-4">
          {/* Search – read-only placeholder, không làm gì */}
          <div className="hidden md:flex items-center bg-blue-100 rounded-full px-6 py-2 border border-transparent">
            <input
              type="text"
              placeholder="Search..."
